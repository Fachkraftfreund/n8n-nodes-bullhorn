...
export const jobOrderFields: INodeProperties[] = [
	...
	},
	{
		...
		type: 'string',
		required: true,
		displayOptions: { show: { resource: ['jobOrder'], operation: ['getAll'], searchType: ['query'] } },
		default: 'isDeleted=false',
		description: 'SQL WHERE syntax (e.g. "clientCorporation.id IN (406) AND isDeleted=false")',
	},
	{
		...
		type: 'string',
		required: true,
		displayOptions: { show: { resource: ['jobOrder'], operation: ['getAll'], searchType: ['search'] } },
		default: 'isDeleted:false',
